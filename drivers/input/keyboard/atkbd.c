				   ...
				   unsigned int flags)
{
	...
	if (!atkbd->enabled) {
		printk(KERN_ERR "%s: atkbd is not ready, continue...\n", __func__);
		/* Don't go to out. This fix resuming form suspend after
		 * keyboard pressed. Since atkbd will be initialized again
		 * right after resume, and, a keycode that received within
		 * suspend would arrive during the initialization. Therefore,
		 * the the keycode will be dropped here since atkbd->enabled
		 * is not set before the initialization is finished.
		 */
		// goto out;
	}
