...
static void palmas_power_off(void)
{
	...
	regmap_read(palmas_dev->regmap[0], 0xFA, &_r);
	_r |= 2; //set bit 1 is 1
	printk("palmas_power_off : set pin function is ACOK[%x]\n",_r);
        regmap_write(palmas_dev->regmap[0], 0xFA,_r);

	regmap_read(palmas_dev->regmap[1], 0x16, &_r);
	_r &= 0x7F; //set bit 7 is 0
	printk("palmas_power_off : set ACOK interrupt[%x]\n",_r);
	regmap_write(palmas_dev->regmap[1], 0x16, _r);
			    ...
			    const struct i2c_device_id *id)
{
	...
	ret = palmas_control_update(palmas, PALMAS_LONG_PRESS_KEY, 0x0c, 0x00);
	if (ret < 0) {
		dev_err(palmas->dev, "Long press key update to six seconds fails\n");
		goto err;
	}
